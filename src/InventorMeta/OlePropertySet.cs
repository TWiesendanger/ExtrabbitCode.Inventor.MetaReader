...
namespace InventorMeta
{
    ...
    public static class PropertySet
    {
        ...
        public static bool IsPropertySet(byte[] b) => b.Length >= 2 && b[0] == 0xFE && b[1] == 0xFF;
        ...
        public static ParsedSet Parse(byte[] b)
        {
            ...
            set.Clsid = new Guid(new ReadOnlySpan<byte>(b, 8, 16));
            int cSections = (int)U32(b, 24);
            int p = 28;
            var sectionLocs = new List<(Guid fmt, int off)>();
            for (int i = 0; i < cSections; i++)
            {
                var fmt = new Guid(new ReadOnlySpan<byte>(b, p, 16));
                int off = (int)U32(b, p + 16);
                sectionLocs.Add((fmt, off));
                p += 20;
            }

            foreach (var (fmt, off) in sectionLocs)
            {
                var sec = new Section { FmtId = fmt, FmtName = FmtName(fmt) };
                // section header: cb(4) cProps(4) then cProps * {propid(4) offset(4)}
                int cProps = (int)U32(b, off + 4);
                var entries = new List<(uint id, int o)>();
                int q = off + 8;
                ...
                int codePage = 1252;
                foreach (var (id, o) in entries)
                    if (id == 1) { try { codePage = (short)U16(b, off + o + 4); } catch { } }

                // dictionary (propid 0) maps custom propids -> names
                var dict = new Dictionary<uint, string>();
                foreach (var (id, o) in entries)
                    if (id == 0) ParseDictionary(b, off + o, codePage, dict);

                foreach (var (id, o) in entries)
                {
                    if (id == 0) continue; // dictionary, already handled
                    int vp = off + o;
                    ushort type = (ushort)U16(b, vp);
                    object? val;
                    ...
                    sec.Props.Add(new Prop
                    {
                        ...
                        Type = type,
                        Value = val,
                        Name = dict.TryGetValue(id, out var nm) ? nm : WellKnownPid(fmt, id)
                    });
                }
        ...
        private static void ParseDictionary(byte[] b, int off, int codePage, Dictionary<uint, string> dict)
        {
            ...
            for (uint i = 0; i < cEntries && p + 8 <= b.Length; i++)
            {
                ...
                {
                    int byteLen = len * 2;
                    if (p + byteLen > b.Length) break;
                    name = Encoding.Unicode.GetString(b, p, byteLen).TrimEnd('\0');
                    p += byteLen;
                    if ((p - (off)) % 4 != 0) p = ((p + 3) / 4) * 4; // Unicode dict entries pad to 4 bytes
                }
                else
                {
        ...
        private static object? ReadValue(byte[] b, int vp, int codePage)
        {
            ...
            switch (type)
            {
                ...
                case 65: { int n=(int)U32(b,d); var r=new byte[n]; Array.Copy(b,d+4,r,0,Math.Min(n,b.Length-d-4)); return new Blob("BLOB",r); } // VT_BLOB
                case 71: { int n=(int)U32(b,d); var r=new byte[n]; Array.Copy(b,d+4,r,0,Math.Min(n,b.Length-d-4)); return new Blob("CF",r); }   // VT_CF
                case 72: return new Guid(new ReadOnlySpan<byte>(b, d, 16)); // VT_CLSID
                case 0:  return null;  // VT_EMPTY
                case 1:  return null;  // VT_NULL
                default:
                    if ((type & 0x1000) != 0) return ReadVector(b, vp, (ushort)(type & 0x0FFF), codePage);
                    ...
                    if ((type & 0x2000) != 0) return ReadOverride(b, vp, codePage);
        ...
        private static object ReadOverride(byte[] b, int vp, int codePage)
        {
            ...
            var v = ReadInner(b, vp + 20, codePage);
            if (v != null) return v;
        ...
        private static object? ReadInner(byte[] b, int p, int codePage)
        {
            ...
            switch (vt)
            {
                ...
                {
                    int n = (int)U32(b, p + 4);
                    if (n <= 0 || n > 4096 || p + 8 + n > b.Length) return null;
                ...
                case 5:  return p + 12 <= b.Length ? BitConverter.ToDouble(b, p + 4) : (object?)null; // VT_R8
        ...
        private static object ReadVector(byte[] b, int vp, ushort baseType, int codePage)
        {
            ...
            for (int i = 0; i < count && p < b.Length; i++)
            {
                switch (baseType)
                {
                    case 31: { int n=(int)U32(b,p); items.Add(Encoding.Unicode.GetString(b,p+4,(n-1)*2)); p += 4 + n*2; if((p&3)!=0)p=(p+3)&~3; break; }
                    case 30: { int n=(int)U32(b,p); items.Add(Decode(b,p+4,n,codePage).TrimEnd('\0')); p += 4 + n; if((p&3)!=0)p=(p+3)&~3; break; }
        ...
        {
            if (len <= 0 || o + len > b.Length) return "";
