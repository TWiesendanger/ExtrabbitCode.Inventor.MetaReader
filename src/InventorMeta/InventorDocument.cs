...
namespace InventorMeta
{
    ...
    public sealed class InventorDocument
    {
        ...
        public string DocumentType { get; private set; } = "Unknown";
        public DocKind Kind { get; private set; } = DocKind.Unknown;
        ...
        public sealed class ModelState
        {
            ...
            public List<PropEntry> Properties = new();
        ...
        public List<string> ModelStates { get; } = new();
        ...
        public InventorDocument(string path)
        {
            ...
            DocumentType = InventorProps.DocClass.TryGetValue(RootClsid, out var dt) ? dt : $"Unknown ({RootClsid})";
            ...
            var memberStreams = new Dictionary<string, List<byte[]>>(StringComparer.OrdinalIgnoreCase);
            ...
            foreach (var e in cf.Directory.Where(d => d.Type == 2))
            {
                ...
                var mm = Regex.Match(e.Path, @"^/MemberDocs/([^/]+)/[^/]+$", RegexOptions.IgnoreCase);
                if (e.Path.Contains("/MemberDocs/", StringComparison.OrdinalIgnoreCase))
                {
                    if (mm.Success && PropertySet.IsPropertySet(data))
                    {
                        ...
                        (memberStreams.TryGetValue(st, out var l) ? l : memberStreams[st] = new()).Add(data);
                    }
                    continue;
                }
            ...
            foreach (var kv in memberStreams)
            {
                var ms = new ModelState
                {
                    StorageName = kv.Key,
                    Name = _storageToState.TryGetValue(kv.Key, out var nm) ? nm : kv.Key,
                    IsActive = false
                };
        ...
        private PropertySet.ParsedSet ParsePropertySetInto(byte[] data, List<PropEntry> into, bool allowThumbnail)
        {
            var set = PropertySet.Parse(data);
            foreach (var sec in set.Sections)
            {
                string setName = InventorProps.SetNames.TryGetValue(sec.FmtId, out var sn)
                    ? sn : (sec.FmtName.Length > 0 ? sec.FmtName : sec.FmtId.ToString());

                foreach (var pr in sec.Props)
                {
                    if (pr.Id == 1) continue;            // codepage
                    if (pr.Id == 0x80000000) continue;   // internal flags
                    ...
                    if (sec.FmtId == DesignTrackingFmt && pr.Id == 40 && pr.Value is int ds)
                    ...
                    if (allowThumbnail && pr.Value is PropertySet.Blob cf2 && cf2.Kind == "CF" && Thumbnail == null)
        ...
        private void ParseUFRxDoc(byte[] data, HashSet<string> memberStorages)
        {
            ...
            foreach (var storage in memberStorages)
            {
                ...
                {
                    if (i - d >= 0 && IsState(tokens[i - d]) && usedNames.Add(tokens[i - d])) name = tokens[i - d];
                    else if (i + d < tokens.Count && IsState(tokens[i + d]) && usedNames.Add(tokens[i + d])) name = tokens[i + d];
                }
                if (name != null) _storageToState[storage] = name;
            }
            // the primary/active state name (e.g. "[Primary]") sits in a bracketed token
            var prim = tokens.FirstOrDefault(t => Regex.IsMatch(t, @"^\[[^\]]+\]$"));
            if (prim != null) _primaryName = prim;
            ...
            var tm = Regex.Match(txt, @"Document ([A-Za-z]:\\[^\x00-\x1F]*?\.(?:ipt|iam|idw|ipn)) was created", RegexOptions.IgnoreCase);
            ...
            foreach (Match m in Regex.Matches(txt, @"Model State\d*"))
                if (!ModelStates.Contains(m.Value)) ModelStates.Add(m.Value);
        }
        ...
        private void BuildSummaryInto(List<PropEntry> props, Dictionary<string, string> summary)
        {
            void Put(string label, string setId, uint pid)
            {
                ...
                if (p != null && !summary.ContainsKey(label)) summary[label] = p.Display;
            }
        ...
        private static bool Eq(byte[] b, int o, params int[] sig)
        {
            ...
            for (int i = 0; i < sig.Length; i++) if (b[o+i] != (byte)sig[i]) return false;
            return true;
        }
