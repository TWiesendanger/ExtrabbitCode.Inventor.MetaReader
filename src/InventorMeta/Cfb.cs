...
namespace InventorMeta
{
    ...
    public sealed class CompoundFile : IDisposable
    {
        ...
        public List<DirEntry> Directory { get; } = new();
        ...
        public CompoundFile(string path)
        {
            ...
            if (_data.Length < 512) throw new InvalidDataException("File too small to be a compound file.");
            ...
            foreach (uint s in fatSectors)
            {
                ...
                for (int i = 0; i < SectorSize / 4; i++) Fat[fi++] = U32(off + i * 4);
            }
            ...
            while (ms != ENDOFCHAIN && ms != FREESECT && mGuard++ < numMiniFat + 8)
            {
                ...
                for (int i = 0; i < SectorSize / 4; i++) miniFat.Add(U32(off + i * 4));
                ms = NextFat(ms);
            }
            MiniFat = miniFat.ToArray();

            // ---- Read directory entries ----
            var dirBytes = ReadFatChain(firstDirSect);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (MajorVersion == 3) e.Size &= 0xFFFFFFFF; // high dword reserved in v3
                Directory.Add(e);
            }
        ...
        {
            if (id == NOSTREAM || id >= Directory.Count) return;
            var e = Directory[(int)id];
            WalkTree(e.Left, parentPath, depth);
            e.Path = parentPath + "/" + e.Name;
            e.Depth = depth;
            if (e.Type == 1 && e.Child != NOSTREAM) WalkTree(e.Child, e.Path, depth + 1);
        ...
        {
            if (e.Type != 2) throw new InvalidOperationException("Not a stream entry.");
        ...
        {
            if (size < 0 || size >= b.Length) return b;
            var r = new byte[size];
            Array.Copy(b, r, size);
            return r;
        }
        ...
        private int  SectorOffset(uint s) => (int)((s + 1) * (long)SectorSize);
